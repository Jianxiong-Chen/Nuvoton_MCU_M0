                                  ...
                                  uint32_t u32DutyCycle)
{
    ...
    if(pwm == ((PWM_T *) (((uint32_t)0x40000000) + 0x40000)))
        u32ClkSrc = (((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->CLKSEL1 & ((0x3ul << (4)) << (u32ChannelNum & 2))) >> ((4) + (u32ChannelNum & 2));

    else
        u32ClkSrc = (((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->CLKSEL2 & ((0x3ul << (4)) << (u32ChannelNum & 2))) >> ((4) + (u32ChannelNum & 2));

    switch (u32ClkSrc)
    {
    case 0:
        u32PWM_Clock = (12000000UL);
        break;
    case 1:
        u32PWM_Clock = (32768UL);
        break;
    case 2:
    ...
    while((pwm->INTSTS & (0x1ul << (8)) ) == (0x1ul << (8)));
    pwm->PRES = (pwm->PRES & ~((0xfful << (0)) << ((u32ChannelNum >> 1) * 8))) | (u8Prescale << ((u32ChannelNum >> 1) * 8));
    pwm->CLKSEL = (pwm->CLKSEL & ~((0x7ul << (0)) << (4 * u32ChannelNum))) | (u8Divider << (4 * u32ChannelNum));
    pwm->CTL |= ((0x1ul << (3)) << (u32ChannelNum * 8));
    while((pwm->INTSTS & ((0x1ul << (4)) << u32ChannelNum)) == ((0x1ul << (4)) << u32ChannelNum));
    if(u32DutyCycle == 0)
        *(volatile uint32_t *) (&pwm->DUTY0 + 3 * u32ChannelNum) &= ~(0xfffful << (16));
    else
    {
        *(volatile uint32_t *) (&pwm->DUTY0 + 3 * u32ChannelNum) &= ~(0xfffful << (16));
        *(volatile uint32_t *) (&pwm->DUTY0 + 3 * u32ChannelNum) |= ((u32DutyCycle * (u16CNR + 1) / 100 - 1) << (16));
    }
    *(volatile uint32_t *) (&pwm->DUTY0 + 3 * u32ChannelNum) &= ~(0xfffful << (0));
    *(volatile uint32_t *) (&pwm->DUTY0 + 3 * u32ChannelNum) |= u16CNR;
...
void PWM_Stop (PWM_T *pwm, uint32_t u32ChannelMask)
{
    ...
    for(i = 0; i < (4); i ++)
    {
        if(u32ChannelMask & (1 << i))
        {
            *(volatile uint32_t *) (&pwm->DUTY0 + 3 * i) &= ~(0xfffful << (0));
        }
    }
...
void PWM_DisableCapture (PWM_T *pwm, uint32_t u32ChannelMask)
{
    ...
    for (i = 0; i < (4); i++)
    {
        if ( u32ChannelMask  & (1 << i))
        {
            u32CTLMask |= ((0x1ul << (0)) << (i * 8));
            u32CAPCTLMask |= (((0x1ul << (1)) | (0x1ul << (2))) << (i * 8));
        }
    }
...
void PWM_EnableDeadZone (PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32Duration)
{
    ...
    pwm->PRES = (pwm->PRES & ~((0xfful << (16)) << (8 * u32ChannelNum))) | ((u32Duration << (16) ) << (8 * u32ChannelNum));
    
    pwm->CTL |= ((0x1ul << (4)) << u32ChannelNum);
}
...
uint32_t PWM_GetCaptureIntFlag (PWM_T *pwm, uint32_t u32ChannelNum)
{
    return ((pwm->CAPINTSTS >> (u32ChannelNum * 8)) & ((6UL)));
}
...
void PWM_EnablePeriodInt (PWM_T *pwm, uint32_t u32ChannelNum,  uint32_t u32IntPeriodType)
{
    ...
    pwm->INTEN |= ((0x1ul << (0)) << u32ChannelNum);
}
...
void PWM_ClearPeriodIntFlag (PWM_T *pwm, uint32_t u32ChannelNum)
{
    ...
    pwm->INTSTS = ((0x1ul << (0)) << u32ChannelNum);
}
...
uint32_t PWM_GetPeriodIntFlag (PWM_T *pwm, uint32_t u32ChannelNum)
{
    return ((pwm->INTSTS & ((0x1ul << (0)) << u32ChannelNum)) ? 1 : 0);
}
...
{
    if (u32ChannelNum == 0)
        pwm->CAPCTL = (pwm->CAPCTL & ~((0x3ul << (4)) | (0x1ul << (12)))) | u32Mode | u32RisingFirst | (0x1ul << (3));
    else
        pwm->CAPCTL = (pwm->CAPCTL & ~((0x3ul << (20)) | (0x1ul << (28)))) | (u32Mode << 16)| (u32RisingFirst << 16)| (0x1ul << (19));
}
